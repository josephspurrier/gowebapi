// Package gogen will parse and create files from template pairs.
...
func fromMapToFile(templateFile string, d map[string]interface{}, outputFile string) error {
	...
	// Parse the template
	t, err := template.New(base).Delims(LDelimiter, RDelimiter).Funcs(funcmap()).ParseFiles(templateFile)
	if err != nil {
		return err
	}
...
func parseTemplate(m map[string]interface{}, mapFileBytes []byte) (map[string]interface{}, bool, error) {
	...
	// Parse the template
	t, err := template.New("").Delims(LDelimiter, RDelimiter).Funcs(funcmap()).Parse(string(mapFileBytes))
	if err != nil {
		return nil, true, err
	}
